...
export const addVKontakteAuthSupport = ({app, io, listener, httpPort, httpHost}) => {
    ...
    const callbackURL = `https://${httpHost}:${httpPort}/auth/vkontakte/callback`;
    ...
    const authVKPath = '/auth/vkontakte';

    app.get('/', require('connect-ensure-login').ensureLoggedIn(authVKPath), function (req, res, next) {
        console.log('ensure login /');
        next();
    });

    app.get('/game', require('connect-ensure-login').ensureLoggedIn(authVKPath), function (req, res, next) {
        console.log('ensure game /game');
        next();
    });
