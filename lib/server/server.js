...
export const startServer = ({httpPort, httpHost = '127.0.0.1', inMemory = false}) => {
    ...
    const sensorDataStorage = new ElasticStorage();
    ...
    io.on('connection', async function(socket){
        ...
        socket.on('turn', async ({ value, turnHash, location }) => {
            await game.makeTurn({ playerId, value, location, time: moment() });
            socket.emit('turn', {playerId, value, turnHash});
            //todo remove it, after connection client should resync data by himself
            await updateScores();
        });
        socket.on('accelSensorData', (data) => {
            logger.info(`player: ${playerId} sessionId: ${data.sessionId} rawData:${JSON.stringify(data)}`);
            data.playerId = playerId;
            data.time = moment();
            sensorDataStorage.incAcceleration(data);
        });
