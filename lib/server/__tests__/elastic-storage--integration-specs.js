...
describe('elastic daily stores', () => {
    ...
    it("should store sensor data", async () => {
        ...
        const sensorData = { playerId:'p1', sessionId, time:moment(), acceleration: { x:1.21, y:2.22, z:3.31 }, rotationRate: { alpha:5.1, beta:5.2, gamma:5.3  } };
        await store.incAcceleration(sensorData);
        await store.flush();
        const [actualSensorData] = await store.getAcclerations(sessionId);
        actualSensorData.time.format().should.be.eq(sensorData.time.format());
        //fixme workaround for like match
        actualSensorData.time = null;
        sensorData.time = null;
        actualSensorData.should.be.like(sensorData);
    });
