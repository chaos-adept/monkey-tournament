...
const enterValueInInput = async (driver, css, value) => {
    const enterAttemptInput = css;
...
const enterAttempt = async (driver, value, clan) => {
    ...
    await enterValueInInput(driver, `input[name=attempt-amount-input]`, value);
    await enterValueInInput(driver, `input[name=attempt-clan-input]`, clan);
    await driver.wait(until.elementIsEnabled(driver.findElement(By.css(`[data-sid='confirm-attempt-btn']`))));
...
describe("Integration login actions", () => {
    ...
    before(async () => {
        ...
        localServerUrl = `https://localhost:${httpPort}`;
        localServerSocketIOClientUrl = `https://localhost:${httpPort}/socket.io/socket.io.js`;
        localServerLoginUrl = `https://localhost:${httpPort}/login`;
    ...
    it("should log attempt", async () => {
        //when
        await enterAttempt(driver, '12', 'deadbabyes');
        //when
        const scoreElSelector = `[data-sid="game-result-list"] [data-sid="score-list-item"] [data-sid="score-value"]`;
        await driver.wait(until.elementLocated(By.css(scoreElSelector)), 5000);
