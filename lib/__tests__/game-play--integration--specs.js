...
const loginVkUser = async (driver, localServerUrl) => {
    driver.get(localServerUrl);
    var submit = await driver.findElement(By.xpath(".//*[starts-with(@data-sid, 'login-vk-submit')]"));
    submit.click();

    await driver.wait(async () => {
        var url = await driver.location();
        return url.contains('vk.com');
    }, 10000);

    var userNameInput = await driver.findElement(By.id("quick_email"));
    userNameInput.clear();
    userNameInput.sendKeys('e@e.com');

    var passwordInput = await driver.findElement(By.xpath("quick_pass"));
    passwordInput.clear();
    passwordInput.sendKeys('password');
...
describe("Integration login actions", () => {
    ...
    it("should login players over vk", async function () {
        ...
        try {
            await Promise.all([
                loginVkUser(driver1, localServerUrl, 'jill', 'birthday')]);
