...
import {getOrCreateUser, findUserById} from './users';
const express = require('express');
const passport = require('passport');


export const addVKontakteAuthSupport = (app) => {
    ...
    passport.use(new VKontakteStrategy({
        ...
        function(accessToken, refreshToken, profile, done) {
            ...
            (async() => {
                try {
                    const user = await getOrCreateUser({ provider:'vkontakte', id: profile.id, profile });
...
// will be set at `req.user` in route handlers after authentication.
    passport.use('local', new Strategy(
        function (username, password, cb) {
            (async () => {
                try {
                    const user = await getOrCreateUser({provider: 'local', id: username, profile:{}});
                    if (user.password != password) {
                        throw new Error(`User password is not match ${user.password} / ${password}`);
                    }
                    return cb(null, user);
                } catch (err) {
                    cb(err);
